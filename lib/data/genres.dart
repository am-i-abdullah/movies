import 'package:flutter/material.dart';
import '../models/genre.dart';

List<Genre> genres = [
  const Genre(
    id: 28,
    tag: 'Action',
    tagColor: Colors.red,
    imageURL:
        'https://media.themoviedb.org/t/p/w600_and_h900_bestv2/9GBhzXMFjgcZ3FdR9w3bUMMTps5.jpg',
  ),
  const Genre(
    id: 12,
    tag: 'Adventure',
    tagColor: Colors.green,
    imageURL:
        'https://image.tmdb.org/t/p/w600_and_h900_bestv2/7lTnXOy0iNtBAdRP3TZvaKJ77F6.jpg',
  ),
  const Genre(
    id: 16,
    tag: 'Animation',
    tagColor: Colors.orange,
    imageURL:
        'https://image.tmdb.org/t/p/w600_and_h900_bestv2/ldfCF9RhR40mppkzmftxapaHeTo.jpg',
  ),
  const Genre(
    id: 35,
    tag: 'Comedy',
    tagColor: Colors.deepOrangeAccent,
    imageURL:
        'https://image.tmdb.org/t/p/w600_and_h900_bestv2/qhb1qOilapbapxWQn9jtRCMwXJF.jpg',
  ),
  const Genre(
    id: 80,
    tag: 'Crime',
    tagColor: Colors.brown,
    imageURL:
        'https://www.themoviedb.org/t/p/w600_and_h900_bestv2/gma8o1jWa6m0K1iJ9TzHIiFyTtI.jpg',
  ),
  const Genre(
    id: 99,
    tag: 'Documentary',
    tagColor: Colors.blue,
    imageURL:
        'https://www.themoviedb.org/t/p/w600_and_h900_bestv2/eLbDSu14EZ2VldIgDDGrt5pJkoL.jpg',
  ),
  const Genre(
    id: 18,
    tag: 'Drama',
    tagColor: Colors.purple,
    imageURL:
        'https://www.themoviedb.org/t/p/w600_and_h900_bestv2/cND79ZWPFINDtkA8uwmQo1gnPPE.jpg',
  ),
  const Genre(
    id: 10751,
    tag: 'Family',
    tagColor: Colors.teal,
    imageURL:
        'https://www.themoviedb.org/t/p/w600_and_h900_bestv2/kuf6dutpsT0vSVehic3EZIqkOBt.jpg',
  ),
  const Genre(
    id: 14,
    tag: 'Fantasy',
    tagColor: Colors.pink,
    imageURL:
        'https://image.tmdb.org/t/p/w600_and_h900_bestv2/7lTnXOy0iNtBAdRP3TZvaKJ77F6.jpg',
  ),
  const Genre(
    id: 36,
    tag: 'History',
    tagColor: Colors.grey,
    imageURL:
        'https://image.tmdb.org/t/p/w600_and_h900_bestv2/7lTnXOy0iNtBAdRP3TZvaKJ77F6.jpg',
  ),
  const Genre(
    id: 27,
    tag: 'Horror',
    tagColor: Colors.deepOrange,
    imageURL:
        'https://image.tmdb.org/t/p/w600_and_h900_bestv2/7lTnXOy0iNtBAdRP3TZvaKJ77F6.jpg',
  ),
  const Genre(
    id: 10402,
    tag: 'Music',
    tagColor: Colors.indigo,
    imageURL:
        'https://image.tmdb.org/t/p/w600_and_h900_bestv2/7lTnXOy0iNtBAdRP3TZvaKJ77F6.jpg',
  ),
  const Genre(
    id: 9648,
    tag: 'Mystery',
    tagColor: Colors.deepPurple,
    imageURL:
        'https://image.tmdb.org/t/p/w600_and_h900_bestv2/7lTnXOy0iNtBAdRP3TZvaKJ77F6.jpg',
  ),
  const Genre(
    id: 10749,
    tag: 'Romance',
    tagColor: Colors.lightBlue,
    imageURL:
        'https://image.tmdb.org/t/p/w600_and_h900_bestv2/7lTnXOy0iNtBAdRP3TZvaKJ77F6.jpg',
  ),
  const Genre(
    id: 878,
    tag: 'Science Fiction',
    tagColor: Colors.lightGreen,
    imageURL:
        'https://image.tmdb.org/t/p/w600_and_h900_bestv2/7lTnXOy0iNtBAdRP3TZvaKJ77F6.jpg',
  ),
  const Genre(
    id: 10770,
    tag: 'TV Movie',
    tagColor: Colors.cyan,
    imageURL:
        'https://image.tmdb.org/t/p/w600_and_h900_bestv2/7lTnXOy0iNtBAdRP3TZvaKJ77F6.jpg',
  ),
  const Genre(
    id: 53,
    tag: 'Thriller',
    tagColor: Colors.lime,
    imageURL:
        'https://image.tmdb.org/t/p/w600_and_h900_bestv2/7lTnXOy0iNtBAdRP3TZvaKJ77F6.jpg',
  ),
  const Genre(
    id: 10752,
    tag: 'War',
    tagColor: Colors.amber,
    imageURL:
        'https://image.tmdb.org/t/p/w600_and_h900_bestv2/7lTnXOy0iNtBAdRP3TZvaKJ77F6.jpg',
  ),
  const Genre(
    id: 37,
    tag: 'Western',
    tagColor: Colors.brown,
    imageURL:
        'https://image.tmdb.org/t/p/w600_and_h900_bestv2/7lTnXOy0iNtBAdRP3TZvaKJ77F6.jpg',
  ),
];
